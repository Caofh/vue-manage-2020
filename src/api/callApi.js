...
import { typeOf, getCookie } from '@/utils/index'
...
const callApi = (apiRoot = '/', headers) => {
  ...
  let csrfToken = getCookie('lite_manage_csrfToken')
  ...
  let axiosDefault = {
    ...
    headers: {
      ...
      'X-Requested-With': 'XMLHttpRequest', // 特殊header头
  ...
  obj.interceptors.response.use(res => {
    ...
    if (code === 0) {
      ...
      console.log('---------')
      console.log('问题接口：' + (config.baseURL + config.url))
      console.log('问题返回：没登录或登陆过期')
      console.log(data)
      console.log('---------')

      console.log(window.location.href)
      console.log(gateWay.baseApi + '/course/explore_login?r=' + window.location.href)
...
function goLogin () {
  window.location.href = gateWay.baseApi + '/course/explore_login?r=' + window.location.href
}
