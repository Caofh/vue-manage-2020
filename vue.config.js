...
// 区分环境选择cdn地址
let publicPath = '/adminfront' // 静态资源引用路径
let fontPublicPath = '/adminfront' // 字体图标引用的cdn路径
let imgPublicPath = '/adminfront' // css引用图片的cdn路径（c2c/static/img）
if (processEnv === 'production') {
  publicPath = 'https://lc-static.123.com/adminfront' // 正式环境静态资源css、js等cdn路径
  // fontPublicPath = `https://lc-static.123.com/adminfront` // 正式环境字体图标引用的cdn路径
  fontPublicPath = `https://oemu.123.com/adminfront` // 正式环境字体图标引用的cdn路径
  imgPublicPath = `https://lc-static.123.com/adminfront` // 正式环境css引用图片的cdn路径

} else if (processEnv === 'pre') {
  // publicPath = 'https://oemu.123.net/adminfront' // 正式环境静态资源css、js等cdn路径
  // fontPublicPath = `https://oemu.123.net/adminfront` // 正式环境字体图标引用的cdn路径
  // imgPublicPath = `https://oemu.123.net/adminfront` // 正式环境css引用图片的cdn路径

} else if (processEnv === 'test') {

  // 个人开发机配置
  // publicPath = 'http://10.227.19.49:8080/m' // 测试环境静态资源css、js等cdn路径
  // fontPublicPath = 'http://10.227.19.49:8080/m'
  // imgPublicPath = 'http://10.227.19.49:8080/m'

  // 测试环境配置
  // publicPath = 'http://oemu.123.net/m' // 测试环境静态资源css、js等cdn路径
  // fontPublicPath = 'http://oemu.123.net/m'
  // imgPublicPath = 'http://oemu.123.net/m'
...
// const devServerHost = '10.95.251.241';
...
module.exports = {
    ...
    devServer: {
        disableHostCheck: true, // 跳过hostname检查(https://blog.csdn.net/guzhao593/article/details/85918869)
        host: devServerHost,
        port: devServerPort,
    ...
    lintOnSave: false, // 开发环境开启eslint，测试和线上编辑代码禁止eslint
        ...
        loaderOptions: {
            sass: {
                ...
                prependData: `
                    ...
                    @import "@/assets/css/styleExtra/vab.scss";
                `
            },
            postcss: {
                plugins: [
